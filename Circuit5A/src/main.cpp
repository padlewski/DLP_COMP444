...
void setup() {
  ...
  Serial.println("Enter motor speed (-255 to 255)... ");    //Prompt to get input in the serial monitor.
}

void loop() {

  if (Serial.available() > 0) {         //if the user has entered something in the serial monitor
    val = Serial.parseInt();     //set the motor speed equal to the number in the serial message
    if(val == 0) return;
    motorSpeed = val;
