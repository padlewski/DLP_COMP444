...
void setup() {
  ...
  Serial.begin(9600);
  #endif
  o_canada.playCount = 1;
  pinMode(buzzer.pin, OUTPUT);
}

void loop() {
  while (o_canada.playCount){
    #ifdef _PRINT_
    Serial.print("Playing note");
    Serial.println(o_canada.step);
    #endif
    play(buzzer, &o_canada);
    #ifdef _EXPERIMENT_2_
    if(o_canada.step == 57) o_canada.tempo = 60;
    if(o_canada.step == 77) o_canada.tempo = 30;
    #endif 
  }
  while(true){};
}
